...
// DTOs
import { CreateBattleParentCategoryDto, UpdateBattleParentCategoryDto, BattleParentCategoryDto } from './dto/admin/battle-parent-category.dto';
import { CreateBattleChildCategoryDto, UpdateBattleChildCategoryDto, BattleChildCategoryDto } from './dto/admin/battle-child-category.dto';
import { CreateBattleStageDto, UpdateBattleStageDto, BattleStageDto } from './dto/admin/battle-stage.dto';
...
export class BattleAdminService {
    ...
    constructor(
        @InjectRepository(BattleParentCategory) private readonly parentCategoryRepo: Repository<BattleParentCategory>,
        @InjectRepository(BattleChildCategory) private readonly childCategoryRepo: Repository<BattleChildCategory>,
        @InjectRepository(BattleStage) private readonly stageRepo: Repository<BattleStage>,
        @InjectRepository(Npc) private readonly npcRepo: Repository<Npc>,
    ) { }

    // --- Parent Category Management ---

    async createParentCategory(dto: CreateBattleParentCategoryDto): Promise<BattleParentCategoryDto> {
    ...
    async findAllParentCategories(): Promise<BattleParentCategoryDto[]> {
        const categories = await this.parentCategoryRepo.find({ order: { name: 'ASC' } });
        // 【修正一】使用箭頭函式確保 `this` 上下文正確
        return categories.map(category => this.mapParentToDto(category));
    }

    async updateParentCategory(id: string, dto: UpdateBattleParentCategoryDto): Promise<BattleParentCategoryDto> {
    ...
    async removeParentCategory(id: string): Promise<void> {
        ...
        if (result.affected === 0) throw new NotFoundException(`找不到 ID 為 ${id} 的父分類`);
    }

    // --- Child Category Management ---

    async createChildCategory(dto: CreateBattleChildCategoryDto): Promise<BattleChildCategoryDto> {
        const parent = await this.parentCategoryRepo.findOneByOrFail({ id: dto.parentId });
    ...
    async updateChildCategory(id: string, dto: UpdateBattleChildCategoryDto): Promise<BattleChildCategoryDto> {
        const category = await this.childCategoryRepo.preload({ id, ...dto });
        if (!category) throw new NotFoundException(`找不到 ID 為 ${id} 的子分類`);

        if (dto.parentId) {
            category.parent = await this.parentCategoryRepo.findOneByOrFail({ id: dto.parentId });
        }
    ...
    async removeChildCategory(id: string): Promise<void> {
        ...
        if (result.affected === 0) throw new NotFoundException(`找不到 ID 為 ${id} 的子分類`);
    }
    ...
    async createStage(dto: CreateBattleStageDto): Promise<BattleStageDto> {
        const category = await this.childCategoryRepo.findOneByOrFail({ id: dto.childCategoryId });
        if (!dto.npcId) {
    ...
    async updateStage(id: string, dto: UpdateBattleStageDto): Promise<BattleStageDto> {
        // 在更新時，需要先載入包含 rewards 的完整實體
        const stage = await this.stageRepo.findOne({ where: { id }, relations: ['rewards'] });
        if (!stage) throw new NotFoundException(`找不到 ID 為 ${id} 的對戰關卡`);

        // 使用 preload 來合併純量值
        const updatesToPreload = { ...dto };
        delete updatesToPreload.rewards; // rewards 需要手動處理
        const updatedStage = await this.stageRepo.preload({ id, ...updatesToPreload });
        if (!updatedStage) throw new InternalServerErrorException('Preload failed');

        if (dto.childCategoryId) {
            updatedStage.category = await this.childCategoryRepo.findOneByOrFail({ id: dto.childCategoryId });
        }
        if (dto.npcId) {
        ...
        if (dto.rewards) {
            updatedStage.rewards = dto.rewards.map(r => this.stageRepo.manager.create(BattleReward, { ...r, stage: updatedStage }));
        }
    ...
    async removeStage(id: string): Promise<void> {
        ...
        if (result.affected === 0) throw new NotFoundException(`找不到 ID 為 ${id} 的對戰關卡`);
    }

    // --- Private Mappers ---

    private mapParentToDto(category: BattleParentCategory): BattleParentCategoryDto {
        // 在呼叫此函式前應確保 category 存在，若不存在則是伺服器邏輯錯誤
        if (!category) {
            this.logger.error('mapParentToDto received a null or undefined category.');
            throw new InternalServerErrorException('Mapper function received invalid data.');
        }
        return {
            id: category.id,
            name: category.name,
    ...
    private mapChildToDto(category: BattleChildCategory): BattleChildCategoryDto {
        ...
        if (!category.parent) {
            this.logger.error(`Child category ${category.id} is missing a parent relation.`);
            throw new InternalServerErrorException('Data integrity error: child category is missing a parent.');
        }
        return {
            id: category.id,
            name: category.name,
    ...
    private mapStageToDto(stage: BattleStage): BattleStageDto {
        ...
        if (!stage.category) {
            this.logger.error(`Battle stage ${stage.id} is missing a category relation.`);
            throw new InternalServerErrorException('Data integrity error: stage is missing a category.');
        }
        // 【修正二】明確檢查 npc 是否存在，而不是用 ! 忽略它
        if (!stage.npc) {
            this.logger.error(`Battle stage ${stage.id} is missing an NPC relation.`);
            throw new InternalServerErrorException('Data integrity error: stage is missing an NPC.');
        }
        ...
        return {
            ...
            rewards: (stage.rewards || []).map(reward => this.mapRewardToDto(reward)),
        };
    }
}