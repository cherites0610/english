...
import {
  BattleStageResponse,
  BattleStageListResponse,
} from './dto/admin/battle-stage.response.dto';
...
export class BattleStageAdminController {
  ...
  @Post()
  @ApiCreatedResponse({
    description: '成功創建對戰關卡',
    type: BattleStageResponse,
  })
  async create(
    @Body() createDto: CreateBattleStageDto,
  ): Promise<BattleStageResponse> {
  ...
  @Get()
  @ApiOkResponse({
    description: '成功獲取所有對戰關卡',
    type: BattleStageListResponse,
  })
  ...
  async update(
    @Param('id', ParseUUIDPipe) id: string,
    @Body() updateDto: UpdateBattleStageDto,
  ): Promise<BattleStageResponse> {
