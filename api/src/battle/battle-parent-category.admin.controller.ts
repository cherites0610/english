...
import {
  BattleParentCategoryResponse,
  BattleParentCategoryListResponse,
} from './dto/admin/battle-parent-category.response.dto';
...
export class BattleParentCategoryAdminController {
  ...
  @ApiCreatedResponse({
    description: '成功創建父分類',
    type: BattleParentCategoryResponse,
  })
  async create(
    @Body() createDto: CreateBattleParentCategoryDto,
  ): Promise<BattleParentCategoryResponse> {
  ...
  @Get()
  @ApiOkResponse({
    description: '成功獲取所有父分類',
    type: BattleParentCategoryListResponse,
  })
  ...
  async update(
    ...
    @Body() updateDto: UpdateBattleParentCategoryDto,
  ): Promise<BattleParentCategoryResponse> {
    const category = await this.adminService.updateParentCategory(
      id,
      updateDto,
    );
    if (!category) {
