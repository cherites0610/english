...
import {
  BattleChildCategoryResponse,
  BattleChildCategoryListResponse,
} from './dto/admin/battle-child-category.response.dto';
...
export class BattleChildCategoryAdminController {
  ...
  @ApiCreatedResponse({
    description: '成功創建子分類',
    type: BattleChildCategoryResponse,
  })
  async create(
    @Body() createDto: CreateBattleChildCategoryDto,
  ): Promise<BattleChildCategoryResponse> {
  ...
  @Get()
  @ApiOkResponse({
    description: '成功獲取所有子分類',
    type: BattleChildCategoryListResponse,
  })
  ...
  async update(
    ...
    @Body() updateDto: UpdateBattleChildCategoryDto,
  ): Promise<BattleChildCategoryResponse> {
