...
export class InteractionService {
    ...
    constructor(
        ...
        @InjectRepository(WallMessage) private readonly wallMessageRepo: Repository<WallMessage>,
        @InjectRepository(JournalEntry) private readonly journalEntryRepo: Repository<JournalEntry>,
     ...
     * 新增一條評論，並返回轉換後的 DTO
     */
    async addComment(authorId: string, dto: CreateCommentDto): Promise<CommentDto | null> {
        ...
        if (parentCommentId) {
            ...
            // --- Case 2: 對留言板訊息發表頂層評論 ---
            const wallMessage = await this.wallMessageRepo.findOneByOrFail({ id: wallMessageId });
            newCommentData.rootWallMessage = wallMessage;

        } else if (journalEntryId) {
            // --- Case 3: 對日誌發表頂層評論 ---
            const journalEntry = await this.journalEntryRepo.findOneByOrFail({ id: journalEntryId });
            newCommentData.rootJournalEntry = journalEntry;

        } else {
            // --- Case 4: 未提供任何評論目標 ---
            throw new BadRequestException('必須提供評論目標 (wallMessageId, journalEntryId, 或 parentCommentId)');
        }
    ...
    async toggleLike(userId: string, dto: ToggleLikeDto): Promise<{ liked: boolean; likeCount: number }> {
