...
export class JournalService {
  ...
  async update(
    ...
    const entry = await this.journalRepo.findOneByOrFail({
      id: entryId,
      author: { id: authorId },
    });
  ...
  async remove(authorId: string, entryId: string): Promise<void> {
    const result = await this.journalRepo.delete({
      id: entryId,
      author: { id: authorId },
    });
    ...
    return {
      ...
      comments:
        entry.comments?.map((comment) =>
          this.mapCommentToDto(comment, includeChildrenCount),
        ) || [],
    };
  }
