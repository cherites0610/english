...
import { FurnitureTemplateResponse, FurnitureTemplateListResponse } from './dto/furniture-template.response.dto';
...
export class FurnitureAdminController {
    ...
    @Post()
    @ApiCreatedResponse({ description: '成功創建家具模板', type: FurnitureTemplateResponse })
    async create(@Body() createDto: CreateFurnitureTemplateDto): Promise<FurnitureTemplateResponse> {
        const newTemplate = await this.adminService.create(createDto);
        return { message: '家具模板創建成功', data: newTemplate };
    }

    @Get()
    @ApiOkResponse({ description: '成功獲取所有家具模板', type: FurnitureTemplateListResponse })
    ...
    @Get(':id')
    @ApiOkResponse({ description: '成功獲取單個家具模板', type: FurnitureTemplateResponse })
    ...
    @Patch(':id')
    @ApiOkResponse({ description: '成功更新家具模板', type: FurnitureTemplateResponse })
    @ApiNotFoundResponse({ description: '找不到指定的家具模板' })
    async update(@Param('id') id: string, @Body() updateDto: UpdateFurnitureTemplateDto): Promise<FurnitureTemplateResponse> {
