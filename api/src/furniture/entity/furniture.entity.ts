...
export class Furniture {
    ...
    @ManyToOne(() => User, user => user.furnitureInventory, { onDelete: 'CASCADE' })
    owner: Relation<User>;

    // 修改：與 Hut 的關聯變為可選
    @ManyToOne(() => Hut, hut => hut.furniture, { nullable: true, onDelete: 'SET NULL' })
    hut?: Relation<Hut> | null;

    @ManyToOne(() => FurnitureTemplate, template => template.instances, { eager: true })
