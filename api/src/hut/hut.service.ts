...
export class HutService {
   ...
   */
    ...
    // 將庫存中的家具放置到小屋裡
    async placeFurniture(userId: string, furnitureId: string, dto: { x: number, y: number }): Promise<Furniture> {
        const furniture = await this.furnitureRepository.findOneByOrFail({ id: furnitureId, owner: { id: userId } });
        if (furniture.hut) {
            throw new ConflictException('此家具已被放置');
        }

        const hut = await this.hutRepository.findOneByOrFail({ user: { id: userId } });
        furniture.hut = hut;
        furniture.x = dto.x;
        furniture.y = dto.y;
    ...
    // 將小屋中的家具收回到庫存
    async unplaceFurniture(userId: string, furnitureId: string): Promise<Furniture> {
        const furniture = await this.furnitureRepository.findOneByOrFail({ id: furnitureId, owner: { id: userId } });
        if (!furniture.hut) {
    ...
    // 移動家具（邏輯不變，但權限驗證更明確）
    async moveFurniture(userId: string, furnitureId: string, dto: { x: number, y: number }): Promise<Furniture> {
        const furniture = await this.furnitureRepository.findOneByOrFail({ id: furnitureId, owner: { id: userId }, hut: Not(IsNull()) });
        furniture.x = dto.x;
        furniture.y = dto.y;
        return this.furnitureRepository.save(furniture);
    }
